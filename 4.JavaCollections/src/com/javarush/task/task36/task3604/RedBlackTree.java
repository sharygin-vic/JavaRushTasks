...
public class RedBlackTree {
    ...
    public boolean isEmpty() {
        //return header.left == EMPTY;  //vic
        return header.right == EMPTY;  //vic
    }
    ...
    public void insert(int item) {
        ...
        while (current.element != item) {
            ...
            grand = parent;
            parent = current;
            //current = item > current.element ? current.right : current.left;  //vic
            current = item < current.element ? current.left : current.right;  //vic

            //if (current.left.color == Color.RED && current.right.color == Color.BLACK) {  //vic
            if (current.left.color == Color.RED && current.right.color == Color.RED) {  //vic
                reorient(item);
            }
        }
    ...
    private Node rotateWithRightNode(Node element) {
        Node right = element.right;     //vic
        element.right = right.left;     //vic
        right.left = element;     //vic
        return right;     //vic
    }
